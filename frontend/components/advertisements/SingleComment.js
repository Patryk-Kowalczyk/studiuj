...
const UPDATE_COMMENT = gql`
  mutation UPDATE_COMMENT($id: Int!, $description: String, $rating: Int) {
    ...
    <Grid container wrap="nowrap" spacing={2}>
      ...
      <Grid item xs zeroMinWidth>
        <div style={{ display: "flex" }}>
          <NextLink href={`/user/${comment.user.uuid}`}>
            <h4 className={classes.h4}>{comment.user.name}</h4>
          </NextLink>
          <Rating value={comment.rating / 2} precision={0.5} readOnly />
        </div>
          ...
          <AccordionDetails className={classesCol.details}>
            <Box className={classesCol.root}>
              ...
              <div className={classes.rating}>
                ...
                {editRating / 2 !== null && (
                  <Box ml={3}>
                    {labels[hover !== -1 ? hover : editRating / 2]}
                  </Box>
                )}
              </div>
              <Box
